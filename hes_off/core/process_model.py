...
def evaluate_process_model(HEAT_DEMAND, POWER_DEMAND,
    ...
    WT_energy_loss = np.empty((p,n))         # Energy generated by the wind turbines that are not utilized
    WT_energy_available = np.empty((p,n))
    ...
    for p, (power_demand, heat_demand) in enumerate(zip(POWER_DEMAND, HEAT_DEMAND)):
        ...
        for t in times[p]:
            ...
            WT_energy_loss[p,t] = WT_power_available[t] - WT_power_current
            WT_energy_available[p,t] = WT_power_available[t]
    ...
    result_dict = {"flag":           flag*1.00,     # Conversion from integer to float for Numba
                   ...
                   "WT_energy_loss": WT_energy_loss,
                   "WT_energy_available": WT_energy_available,
...
def compute_GT_power_from_heat(model, number_of_units, heat_output):
    ...
    if model == 'LM2500+G4':
        ...
        if TITAN130_unit_heat*number_of_units < heat_output:
            raise Exception("The given number of gas turbines are not able to cover the heat demand.")
        unit_power = TITAN130_unit_power
        rated_power = unit_power*number_of_units
